...
- Speed, acceleration, and turn-rate commands are saturated by `DubinsVehicle` using the car model's `maxSpeed`, `acceleration`, and handling-derived turn limits. PD and MPCC share the same cruise target (`maxSpeed`, curvature-derated) so traffic-triggered MPCC cannot permanently outrun a clear-road PD loner.
