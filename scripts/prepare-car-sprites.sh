#!/usr/bin/env bash
# Slices src/sprites/cars.png (3x3 grid) into race and menu car sprites,
# replaces cyan-ish background with a soft alpha matte (color unmix),
...
def save_sprite(image: Image.Image, file_name: str) -> None:
	for directory in (out_dir, bundled_sprite_dir):
		image.save(directory / file_name, format="PNG")
...
config_lines = [
	...
	"# Fields: index, number, name, mean color, race sprite size, and stats",
